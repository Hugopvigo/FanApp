...
object GoodreadsCsvParser {
    ...
    fun parse(inputStream: InputStream): ImportPreview {
        ...
        return reader.use { r ->
            ...
            if (lines.size < 2) return@use ImportPreview(0, 0, 0, 0, 0, emptyList())

            val headers = parseCsvLine(lines[0])
            val titleIdx = headers.indexOf(COLUMN_TITLE).takeIf { it >= 0 }
                ?: return@use ImportPreview(0, 0, 0, 0, 0, emptyList())
